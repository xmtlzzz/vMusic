...
import { buildSceneChips, buildStageRows, formatTime } from "../utils/visual.js";

export const PlayerStage = defineComponent({
  name: "PlayerStage",
  props: {
  ...
  emits: ["toggle-like", "toggle-play", "previous", "next", "seek"],
  setup(props) {
    const sceneChips = computed(() => buildSceneChips(props.currentTrack, {
      liked: props.currentTrackLiked,
    }));

    const stageRows = computed(() => buildStageRows(props.currentTrack, {
      liked: props.currentTrackLiked,
      playing: props.player.isPlaying,
    }));

    const progressValue = computed(() => props.player.currentTime || 0);
      ...
      <div class="player-stage__grid">
        ...
        <div class="player-stage__right">
          <div class="player-stage__headline">
            ...
            <p class="player-subcopy">{{ currentTrack ? currentTrack.author : '这里会展示当前歌曲信息、播放进度和收藏状态。' }}</p>
          </div>

          <div class="scene-chip-list">
            <div v-for="chip in sceneChips" :key="chip.label" class="scene-chip">
          ...
          <div class="stage-rows">
            <div v-for="row in stageRows" :key="row" class="stage-row">{{ row }}</div>
          </div>

          <div class="stage-footer">
            <span class="stage-footer__count">当前队列 {{ queueCount }} 首</span>
            <span class="stage-footer__file">{{ currentTrack && currentTrack.file_name ? currentTrack.file_name : '等待音频输入' }}</span>
          </div>
        </div>
      </div>
    </section>
